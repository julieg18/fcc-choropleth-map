...
const svg = d3.select('svg').attr('height', 615).attr('width', 950);
...
firstReq.onload = function () {
  ...
  secondReq.onload = function () {
    ...
    let geojson = topojson.feature(
      countyJSONdata,
      countyJSONdata.objects.counties,
    );
    ...
    function getLegendTextArray() {
      let minNum = parseFloat(
        d3.min(educationJSONdata, (d) => d.bachelorsOrHigher).toFixed(1),
      );
      let maxNum = parseFloat(
        d3.max(educationJSONdata, (d) => d.bachelorsOrHigher).toFixed(1),
      );
      let arrayNum = minNum;
      let averageNum = parseFloat(
        ((maxNum - minNum) / (colors.length + 1)).toFixed(1),
      );
    ...
    function getEducationData(countyData, educationData) {
      return educationJSONdata.filter(
        (elem) => elem.fips === countyData['id'],
      )[0][educationData];
    }
    ...
    svg
      ...
      .attr('data-education', (d, i) =>
        getEducationData(d, 'bachelorsOrHigher'),
      )
      .attr('fill', (d, i) =>
        colorScale(getEducationData(d, 'bachelorsOrHigher')),
      )
      // tooltip
      .on('mouseover', (d, i) => {
        d3.select('#tooltip')
          ...
          .html(
            `${getEducationData(d, 'area_name')}, ${getEducationData(
              d,
              'state',
            )} ${getEducationData(d, 'bachelorsOrHigher')}%`,
          );
